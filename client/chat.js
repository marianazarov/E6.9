...
socket.onclose = async (event) => {
    console.error(event);
    window.alert('НЕТ СВЯЗИ С СЕРВЕРОМ DJANGO!');
    console.log('НЕТ СВЯЗИ С СЕРВЕРОМ DJANGO!');
};
...
function printUserCard(item) {
    ...
    <div class="div">
        <img src="${item.avatar}">
        <br>
        <strong>Изменить аватар:</strong><br>
...
async function editAvatar(userId) {
    ...
    if (fileField.files[0]) {
        ...
        try {
            ...
            console.log('Фото сохранено:', JSON.stringify(result));
        } catch (error) {
...
function printChat(data) {
    divChat.innerHTML = `<div class="room_name" style="text-align: center;">Комната: ${data['MessageList']}</div>
    ...
    // Принимаем входящие сообщения от подключенной комнаты
